...
function loadRalphSettings(): RalphSettings {
	try {
		...
		if (settings?.ralph && typeof settings.ralph.maxIterations === "number") {
			return { maxIterations: settings.ralph.maxIterations };
		}
	} catch {
...
let _ralphSettings: RalphSettings | null = null;
function getRalphSettings(): RalphSettings {
	if (!_ralphSettings) _ralphSettings = loadRalphSettings();
	return _ralphSettings;
}
...
───────────────────────────────────────────────────────────────────────`;
	...
	function parseArgs(argsStr: string) {
		...
		const defaults = getRalphSettings();
		const result = {
			name: "",
  ...
  /ralph start review --items-per-iteration 5 --reflect-every 10`;
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			maxIterations: Type.Optional(Type.Number({ description: "Max iterations (default from settings)" })),
		}),
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			const state: LoopState = {
				...
				taskFile,
				iteration: 1,
				maxIterations: params.maxIterations ?? getRalphSettings().maxIterations,
